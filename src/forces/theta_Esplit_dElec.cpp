...
theta_Esplit_dElec::theta_Esplit_dElec(int num_states, int elec_beads, double alpha,
...
void theta_Esplit_dElec::update_theta_dElec(const matrix<std::complex<double> > & x,
...
void theta_Esplit_dElec::update_theta_dx_vec(const matrix<std::complex<double> > & x,
                                            const matrix<std::complex<double> > & p){
...
void theta_Esplit_dElec::update_theta_dp_vec(const matrix<std::complex<double> > & x,
                                            const matrix<std::complex<double> > & p){
...
void theta_Esplit_dElec::update_f_chain(){
    
    f_chain[0] = identity_matrix<std::complex<double> > (num_states);
    
    for (int bead=1; bead<elec_beads; bead++) {
        noalias(fchain_temp) = prod(C->get_C_alpha(bead-1),M->get_M_alpha(0));
        f_chain[bead] = prod(f_chain[bead-1], fchain_temp);
    }
}
void theta_Esplit_dElec::update_b_chain(){
    
    b_chain[elec_beads-1] = M->get_M_alpha(0);
    
    for (int bead = elec_beads - 2; bead > -1; bead--) {
        noalias(bchain_temp) = prod(M->get_M_alpha(0),C->get_C_alpha(bead+1));
        b_chain[bead] = prod(bchain_temp, b_chain[bead+1]);
    }
}
const matrix<double> & theta_Esplit_dElec::get_theta_dx_vec(){
    return theta_dx_vec;}
const matrix<double> & theta_Esplit_dElec::get_theta_dp_vec(){
    return theta_dp_vec;}
